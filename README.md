...
(`backend/app/scoring.py`) - an LLM only ever produces sub-signals that feed into it.
...
Three keys are required to run analyses. All have free tiers and none require a credit card to
start (except OpenAI, which is pay-as-you-go but cheap per request).
...
Two more are optional - the app works fully without them, and those features simply stay hidden
until configured:
...
Tables are created automatically on startup (`Base.metadata.create_all` - no migration tool).
...
searches Exa, and scrapes up to 5 related articles with Firecrawl. Re-analyzing the same URL
within 6 hours returns instantly from cache (see [Optional features](#optional-features) below).
...
- **Headline/body consistency** and **sensationalism** - signals produced by the LLM, passed
  through as-is (they are sub-signals, not the final score).
...
By design, this project does not include: Docker, a database migration tool, user accounts, a
browser extension, a from-scratch ML model, a comprehensive global source-rating database, a
dedicated AI-image forensics system, or any claim of definitively "fake" vs "real". Richer source
history and authentication are unimplemented follow-ups beyond current scope.
